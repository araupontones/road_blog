...
exit_folder = file.path(dir_charts, "Meca")
location = "Canos de Meca"
date_start = ymd("2020-10-25") #Date that the trip started to this location
date_post = ymd("2020-11-15") #Date when you wrote the post
...
sum(clean_data$casos_nuevos, na.rm = T)

#read data: from  https://cnecovid.isciii.es/covid19/resources/datos_ccaas.csv

raw_data = import(file.path(dir_datos, "datos_ccaas_15.11.csv"))
...
geom_col(width = .3,
  ...
  geom_text_repel(
    ...
    aes(label = c("Uncastillo", "Cinfaes", "Etorraberri", "Bemposta"),
        x = fecha,
        y = casos_7),
    nudge_x = -30,
    nudge_y = 1000,
  ...
  labs(x = "",
       ...
       caption = '**Chart:** @AndresArau | **Data:**_https//cnecovid.isciii.es/covid19_  | **15 Nov,2020**') +
