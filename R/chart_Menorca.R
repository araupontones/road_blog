...
exit_folder = file.path(dir_charts, "Menorca")
location = "Menorca"
date_start = ymd("2021-05-15") #Date that the trip started to this location
date_post = ymd("2021-06-01") #Date when you wrote the post
...
#labels -----------------------------------------------------------------
  geom_text_repel(
    ...
    aes(label = c("Uncastillo", "Cinfaes", "Etorraberri", "Bemposta", 
                  "Caños de Meca", "Hortichuelas", "Panticosa"),
        x = fecha,
        y = casos_7),
    nudge_x = c(rep(-50,5),30, -30),
    nudge_y = c(rep(1000,5),2000, 3000),
    angle = c(rep(0,5),0, 0),
    segment.size  = .5,
    family = "Roboto",
    size = 4
  ...
  labs(x = "",
       ...
       caption = '**Chart:** @AndresArau | **Data:**_https//cnecovid.isciii.es/covid19/resources_ | **6 June,2021**') +
