...
location = "Uncastillo"
date_start = ymd("2020-03-14") #Date that the trip started to this location
date_post = ymd("2020-09-13") #Date when you wrote the post
...
#File to export
exfile = file.path(dir_charts, paste0("chart_",location,".png"))
...
casos_dia_llegamos = data_clean$confirmed[data_clean$date==date_start]
casos_today = data_clean$confirmed[data_clean$date==date_post]
...
chart_for_blog = ggplot(data = data_clean,
  ...
  geom_line(data = data_before,
            color = light_purple,
            size = 1.2
            ) +
##Line after
  geom_line(data = data_after,
            color = grey,
            linetype = "dashed",
            size = 1.3)+
